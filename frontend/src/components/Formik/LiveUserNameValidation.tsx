...
async function validateSingupUsername(
    username: string
): Promise<{ is_username_unique: boolean }> {
    const res = await fetch(`/api/auth/validate_signup_username/${username}`);
    const isValid = await res.json();
    return isValid;
}

async function validateLoginUsername(
    username: string
...
export default function LiveUsernameValidation({
    label,
    ...props
}: FieldProps) {
    const {
